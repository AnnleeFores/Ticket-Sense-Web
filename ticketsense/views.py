...
def verifyUser(request):
    ...
    if request.method == "POST":
        ...
        tguser, created = TGuser.objects.update_or_create(
            id=id,
...
def trigger(request):
    ...
    if request.method == "POST":
        ...
        response = requests.get(
            f"https://api.themoviedb.org/3/search/movie?api_key={TMDB_API_KEY}&language=en-US&query={movie}&page=1&include_adult=false&primary_release_year={release_year}"
        ).json()
        try:
...
def bms_theatre(request, location):
    response = (
        scraper.get(
            f"https://in.bookmyshow.com/pwa/api/de/venues?regionCode={location}&eventType=MT&bmsId=1.763873945.1666881673656"
        )
    ).json()
