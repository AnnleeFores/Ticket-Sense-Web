...
from lib.stringMatching import stripDown
...
def get_tktnew_data(location):
    response = requests.get(
        f'{scrapsense_link}/crawl.json?spider_name=tkdata&start_requests=true&crawl_args={{"location":"{location}"}}'
    ).json()
    try:
        data = response["items"]
    except:
...
def store_tktnew_data(response, location):
    tktnewData, created = TktnewData.objects.update_or_create(
        location=location, data=response, defaults={"location": location}
    )
...
def five_min_func():
    try:
        ...
        for trigger in triggers:
            ...
            filmkeyword = (
                trigger.movie
            ).lower()  # make sure that this is lowercase otherwise the result will be incorrect
...
def fetch(link, filmkeyword, date, site, pk, USER_ID, poster, venuecode):
    ...
    if site == "tk":
        response = requests.get(
            f'{scrapsense_link}/crawl.json?spider_name=tk&start_requests=true&crawl_args={{"link":"{link}","film":"{filmkeyword}","date":"{date}"}}'
        ).json()
        try:
            data = response["items"]
        except:
        ...
        response = scraper.get(
            f"https://in.bookmyshow.com/api/v2/mobile/showtimes/byvenue?venueCode={venuecode}&dateCode={date}"
        ).json()
        try:
            bmsdata = response
        except:
        ...
        if bmsdata["ShowDetails"][0]["Date"] == date and bmsdata != "":
            for films in bmsdata["ShowDetails"][0]["Event"]:
                ...
                if (
                    fuzz_value >= 65
                ):  # a threshold value based on Levenshtein distance comparison of 2 strings. Make changes to threshold value to work as you needed
