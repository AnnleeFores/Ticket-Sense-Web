...
def verify_telegram_authentication(bot_token, request_data):
    ...
    for data_pair in request_data_alphabetical_order:
        key, value = str(data_pair[0]), str(data_pair[1])
    ...
    if unix_time_now - unix_time_auth_date > ONE_DAY_IN_SECONDS:
        raise Exception('''Authentication data is outdated. Authentication was received more than day ago.''')


    if _hash != received_hash:
        raise Exception('''This is not a Telegram data. Hash from recieved authentication data does not match
